...
- [scripts/syncYouthPolicies.js](../scripts/syncYouthPolicies.js) — `npm run sync-policies`로 실행하는
  ...
  - **지역/자격 판정 버그 3건 추가 수정(2026-08-23, 사용자가 실제 공고 3개를 지목해서 발견)**:
    ...
    2. **"전남광주통합특별시"가 옛 지역명(광주광역시)으로 잘못 잡히던 문제** — 처음엔 이 이름을
       ...
       **알려진 한계**: 회원가입/프로필 화면의 거주지 선택([lib/profileFields.ts](../../../lib/profileFields.ts)의
       `PROVINCE_OPTIONS`)은 아직 옛 "광주광역시"/"전라남도"를 그대로 쓰고 있어서, 실제로 이 통합
       지역에 사는 사용자가 프로필에서 고를 수 있는 지역명과 정책에 저장된 "전남광주통합특별시"가
       문자열로 안 맞음 — 이 지역 사용자의 자격 판정/필터가 정확하지 않을 수 있음. 프로필 쪽
       선택지 구조(현재 metro/province 두 종류뿐이라 통합 지역처럼 구/시가 섞인 경우를 못 담음)를
       고쳐야 하는 별도 작업이라 이번엔 손 안 댐 — 다음에 필요하면 진행.
